 ...
 */
class BlockdownpdfActivity extends ObjectModel
{
    ...
    /**
     * @param array $bld_languages
     * @param int $type_link
     * @param int $id_cms
     */
    public function handleBlockValues($bld_languages, $type_link, $id_cms)
    {
        $languages = Language::getLanguages();
        $newValues = [];

        foreach ($bld_languages as $key => $value) {
            $newValues[$key] = [
                'title' => $value->title,
    ...
    public static function getAllBlock()
    {
        ...
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'blockdownpdf` pr
            LEFT JOIN ' . _DB_PREFIX_ . 'blockdownpdf_lang prl ON (pr.id_blockdownpdf = prl.id_blockdownpdf)
            ORDER BY pr.position';

        $dbResult = Db::getInstance()->executeS($sql);

        foreach ($dbResult as $key => $value) {
            if (!isset($result[$value['id_blockdownpdf']])) {
                $result[$value['id_blockdownpdf']] = $value;
                $result[$value['id_blockdownpdf']]['title'] = [];
                $result[$value['id_blockdownpdf']]['description'] = [];
                $result[$value['id_blockdownpdf']]['url'] = [];
            }
            $result[$value['id_blockdownpdf']]['title'][$value['id_lang']] = $value['title'];
            $result[$value['id_blockdownpdf']]['description'][$value['id_lang']] = $value['description'];
            $result[$value['id_blockdownpdf']]['url'][$value['id_lang']] = $value['link'];
        }
