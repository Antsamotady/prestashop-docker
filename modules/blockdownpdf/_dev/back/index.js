...
$(window).ready(() => {
  ...
  new Sortable(document.getElementById('list-blockdownpdf'), {
    animation: 150,
    ...
    onUpdate() {
      ...
      $.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: window.bld_controller_block_url,
        data: {
          ajax: true,
  ...
  $(document).on('click', '.blv-listing-row .switch-input', (e) => {
    ...
    $(e.target).parent().find('.bld_switch_text').hide();
    if (switchIsOn) {
    ...
    $.ajax({
      ...
      async: false,
      data: {
        controller: window.bld_controller_block,
        action: 'changeBlockStatus',
        idpsr: $(e.target).parent().attr('data-cart_blockdownpdf_id'),
        status,
        ajax: true,
      },
      success: (data) => {
  ...
  $(document).on('click', 'body', (e) => {
    ...
    const isPopin = $(e.target).closest('#blockdownpdf_block').length;

    if (!isInside && !isPopin) {
      $('#blockdownpdf_block').fadeOut(300);
    }
  });

  // Tab Content : Edit : Modify icon : Tabs
  $(document).on('click', '#blockdownpdf_block .category_select div img', (e) => {
    const category = $(e.target).attr('data-id');
    // Change the tab
    $('#blockdownpdf_block .category_select div').removeClass('active');
    $(e.target).parent().addClass('active');
    // Change the tab content
    $('#blockdownpdf_block .category_blockdownpdf').removeClass('active');
    $(`#blockdownpdf_block .cat_${category}`).addClass('active');
  });

  // Tab Content : Edit : Select icon
  $(document).on('click', '#blockdownpdf_block .category_blockdownpdf .svg', (e) => {
    const svg = $(e.target)[0].outerHTML;

    // Popin : select the icon
    $('#blockdownpdf_block .category_blockdownpdf img.svg.selected').removeClass('selected');
  ...
  $(document).on('click', '#blockdownpdf_block .select_none', () => {
    const psrPicto = $('.psr-picto:visible');
    psrPicto.attr('src', 'undefined').hide();

    // Un-select icon in the popin
    $('#blockdownpdf_block .category_blockdownpdf img.svg').removeClass('selected');
  ...
  $(document).on('change', 'input[name^="BLD_REDIRECTION_"]', (e) => {
  ...
  $(document).on('click', '#saveContentConfiguration', function editTabContentSave() {
    ...
    $('.show-rea-block.active .content_by_lang').each((index, elem) => {
      ...
      if (!minimalData && lang === window.bld_lang && dataType === 'title' && dataToSave[lang][dataType].length > 0) {
        minimalData = true;
      }
    });
    ...
    formData.append('typelink', $(`input[name="BLD_REDIRECTION_${blockId}"]:checked`).val());
  ...
  new Vue({
    ...
    methods: {
      ...
      isActive(item) {
        ...
        $('.bld_menu').addClass('addons-hide');
        $(`.bld_menu#${item}`).removeClass('addons-hide');
  ...
  $(document).on(
    'change',
    'input[name="BLD_HOOK_CHECKOUT"],input[name="BLD_HOOK_HEADER"],input[name="BLD_HOOK_FOOTER"],input[name="BLD_HOOK_PRODUCT"]',
    function updatePosition() {
      let selector;

      switch ($(this).attr('name')) {
        case 'BLD_HOOK_CHECKOUT':
          selector = 'checkout';
          break;
        case 'BLD_HOOK_HEADER':
          selector = 'header';
          break;
        case 'BLD_HOOK_FOOTER':
          selector = 'footer';
          break;
        case 'BLD_HOOK_PRODUCT':
          selector = 'product';
          break;
        default:
          selector = '';
