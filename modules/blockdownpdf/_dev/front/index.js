...
$(window).ready(() => {
  ...
  function styleSVG(imgSrc) {
    const imgTarget = $(`.blockdownpdf_product img.svg.invisible[src="${imgSrc}"], .blockdownpdf img.svg.invisible[src="${imgSrc}"]`);
    ...
    $.ajax({
      ...
      success(data) {
        if ($.isXMLDoc(data)) {
          ...
          $svg.find('path[fill]').attr('fill', window.bld_icon_color);
          $svg.find('path:not([fill])').css('fill', window.bld_icon_color);
  ...
  const imgSrcSvg = $('.blockdownpdf_product img.svg, .blockdownpdf img.svg').map(function getSrcAttr() {
