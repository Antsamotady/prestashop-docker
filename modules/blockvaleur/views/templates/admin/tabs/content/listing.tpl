...
<div id="reminder_listing"
    class="panel panel-default panel-blockvaleur col-lg-10 col-lg-offset-1 col-xs-12 col-xs-offset-0">
    <div class="panel-heading">
        {l s='Block Content' d='Modules.Blockvaleur.Admin'}
    ...
    <div class="panel-body first-body">
        <div class="clearfix">
            <div class="listing-table col-xs-12">
                <div class="listing-head row">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4 hidden-xs content-header">{$fields_captions.description}
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs content-header">{$fields_captions.redirection}
                    </div>
                    <div class="col-lg-2 col-md-2 col-sm-2 hidden-xs content-header">{$fields_captions.actions}</div>
                </div>
                <div id="list-blockvaleur" class="listing-body col-lg-12 col-xs-12">
                    {foreach from=$allblock item=$block key=$key}
                        <div class="listing-general-rol row" data-block="{$block.id_blockvaleur}">
                            <div class="blv-listing-row row">
                                ...
                                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-8">
                                    ...
                                    {else}
                                        {l s='none' d='Modules.Blockvaleur.Admin'}
                                    {/if}
                                </div>
                                <div class="hidden-lg hidden-md hidden-sm col-xs-4">
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-8">
                                    ...
                                    {elseif $block['type_link'] == $LINK_TYPE_URL}
                                        {l s='Url link' d='Modules.Blockvaleur.Admin'}
                                    {elseif $block['type_link'] == $LINK_TYPE_CMS}
                                        {l s='CMS Page' d='Modules.Blockvaleur.Admin'}
                                    {/if}
                                </div>
                                <div class="hidden-lg hidden-md hidden-sm col-xs-4">
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-8 inline-flex">
                                    <label class="col-lg-12 col-xs-12 status-toggle"
                                        id="reminder_active_{$block['id_blockvaleur']}"
                                        for="reminder_active_{$block['id_blockvaleur']}"
                                        data-cart_blockvaleur_id='{$block['id_blockvaleur']}'>
                                        <section class="switch-input {if $block['status']}-checked{/if}">
                                            <input data-toggle="switch" class="switch-new" data-inverse="true"
                                                type="checkbox" name="reminder_active_{$block['id_blockvaleur']}"
                                                checked="">
                                        </section>
                                        <span class="blv_switch_text switch-on"
                                            style="{if !$block['status']}display:none;{/if}">{l s='Activated' d='Modules.Blockvaleur.Admin'}</span>
                                        <span class="blv_switch_text switch-off"
                                            style="{if $block['status']}display:none;{/if}">{l s='Deactivated' d='Modules.Blockvaleur.Admin'}</span>
                                    </label>
                                    <div class="btn-group">
                                        <button class="btn btn-link dropdown-toggle" type="button" data-toggle="dropdown"
                                            aria-haspopup="true" aria-expanded="false"></button>
                                        <div class="dropdown-menu" x-placement="bottom-start">
                                            <span class="dropdown-item psre-edit" data-id="{$block['id_blockvaleur']}"><i
                                                    class="material-icons">mode_edit</i>
                                                {l s='Edit' d='Admin.Actions'}</span>
                                            <span class="dropdown-item psre-delete" data-id="{$block['id_blockvaleur']}"><i
                                                    class="material-icons">delete</i>
                                                {l s='Delete' d='Admin.Actions'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="msgRecommendation">
            {l s='We recommend 3 blocks at maximum.' d='Modules.Blockvaleur.Admin'}
        </div>
    </div>
</div>