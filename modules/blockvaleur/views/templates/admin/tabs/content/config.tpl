...
<div id="blockDisplay"
    class="panel panel-default panel-blockvaleur col-lg-10 col-xs-10 col-lg-offset-1 col-xs-offset-1 inactive">
    <div class="panel-heading">
        {l s='Blockvaleur Block' d='Modules.Blockvaleur.Admin'}
    </div>
    {include file="./config_elements/blockvaleur_block.tpl"}
    ...
    {foreach from=$allblock item=$block key=$key}
        <form class="form_{$block['id_blockvaleur']}" method="post" ENCTYPE="multipart/form-data">
            <div class="panel-body-{$block['id_blockvaleur']} panel-body show-rea-block inactive">
                {* icon *}
                {include file="./config_elements/icon.tpl"}
                {* language *}
                {include file="./config_elements/language.tpl"}
    ...
    <div class="panel-footer">
        <div class="col-xs-6 text-left">
            <input name="refreshPage" type="submit" class="btn btn-primary refreshPage"
                value="{l s='Return' d='Modules.Blockvaleur.Admin'}">
        </div>
        <div class="col-xs-6 text-right">
            <input name="saveContentConfiguration" id="saveContentConfiguration" data-id="" type="submit"
                class="btn btn-primary" value="{l s='Save' d='Modules.Blockvaleur.Admin'}">
        </div>
    </div>
</div>