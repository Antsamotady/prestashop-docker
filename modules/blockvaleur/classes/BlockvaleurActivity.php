 ...
 */
class BlockvaleurActivity extends ObjectModel
{
    ...
    /**
     * @param array $blv_languages
     * @param int $type_link
     * @param int $id_cms
     */
    public function handleBlockValues($blv_languages, $type_link, $id_cms)
    {
        $languages = Language::getLanguages();
        $newValues = [];

        foreach ($blv_languages as $key => $value) {
            $newValues[$key] = [
                'title' => $value->title,
    ...
    public static function getAllBlock()
    {
        ...
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'blockvaleur` pr
            LEFT JOIN ' . _DB_PREFIX_ . 'blockvaleur_lang prl ON (pr.id_blockvaleur = prl.id_blockvaleur)
            ORDER BY pr.position';

        $dbResult = Db::getInstance()->executeS($sql);

        foreach ($dbResult as $key => $value) {
            if (!isset($result[$value['id_blockvaleur']])) {
                $result[$value['id_blockvaleur']] = $value;
                $result[$value['id_blockvaleur']]['title'] = [];
                $result[$value['id_blockvaleur']]['description'] = [];
                $result[$value['id_blockvaleur']]['url'] = [];
            }
            $result[$value['id_blockvaleur']]['title'][$value['id_lang']] = $value['title'];
            $result[$value['id_blockvaleur']]['description'][$value['id_lang']] = $value['description'];
            $result[$value['id_blockvaleur']]['url'][$value['id_lang']] = $value['link'];
        }
