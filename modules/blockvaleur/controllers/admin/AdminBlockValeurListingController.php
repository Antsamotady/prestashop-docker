 ...
 */
class AdminBlockValeurListingController extends ModuleAdminController
{
    /** @var blockvaleur */
    public $module;
    ...
    {
        $now = new DateTime();
        $blockvaleurId = (int) Tools::getValue('idpsr');
        ...
        $whereCondition = 'id_blockvaleur = ' . $blockvaleurId;

        $updateResult = Db::getInstance()->update('blockvaleur', $dataToUpdate, $whereCondition);
    ...
    public function displayAjaxDeleteBlock()
    {
        ...
        $blockPSR = Db::getInstance()->getRow('SELECT * FROM ' . _DB_PREFIX_ . 'blockvaleur WHERE `id_blockvaleur` = ' . (int) $idPSR);
        if (!empty($blockPSR)) {
            $result = true;
            // Remove Custom icon
            ...
            if ($result) {
                $result = Db::getInstance()->delete('blockvaleur_lang', 'id_blockvaleur = ' . (int) $idPSR);
            }
            // Remove Block
            if ($result) {
                $result = Db::getInstance()->delete('blockvaleur', 'id_blockvaleur = ' . (int) $idPSR);
            }
        }
    ...
    public function displayAjaxSaveColor()
    {
        ...
        if (!empty($color1) && !empty($color2)) {
            $result = Configuration::updateValue('BLV_ICON_COLOR', $color1)
                && Configuration::updateValue('BLV_TEXT_COLOR', $color2);
        }
    ...
    public function displayAjaxSaveBlockContent()
    {
        ...
        $blv_languages = (array) json_decode(Tools::getValue('lang_values'));
        ...
        $blockPsr = new BlockvaleurActivity($id_block);
        if (!$id_block) {
            // Last position
            $blockPsr->position = (int) Db::getInstance()->getValue('SELECT MAX(position) AS max FROM ' . _DB_PREFIX_ . 'blockvaleur');
            $blockPsr->position = $blockPsr->position ? $blockPsr->position + 1 : 1;
            $blockPsr->status = false;
        }
        $blockPsr->handleBlockValues($blv_languages, $type_link, $id_cms);
        ...
        if (!empty($_FILES)) {
            ...
            if (version_compare(_PS_VERSION_, '1.7.7.0', '>=')) {
                ...
                // PrestaShop < 1.7.7
                $validUpload = false;
                $mimeType = BlockvaleurActivity::getMimeType($customImage['tmp_name']);
                if ($mimeType && (
    ...
    public function displayAjaxUpdatePosition()
    {
        ...
        if (!empty($blocks) && is_array($blocks)) {
            ...
            foreach ($blocks as $key => $id_block) {
                // Set the position of the Blockvaleur block
                $position = $key + 1;

                $dataToUpdate = ['position' => (int) $position];
                $whereCondition = 'id_blockvaleur = ' . (int) $id_block;
                $updateResult = (bool) Db::getInstance()->update('blockvaleur', $dataToUpdate, $whereCondition);
    ...
    private function isAuthorizedHookConfigurationKey($hook)
    {
        return
            !empty($hook) &&
            in_array($hook, [
                blockvaleur::BLV_HOOK_HEADER,
                blockvaleur::BLV_HOOK_FOOTER,
                blockvaleur::BLV_HOOK_PRODUCT,
                blockvaleur::BLV_HOOK_CHECKOUT,
            ], true)
        ;
    }
    ...
    private function isAuthorizedPositionValue($value)
    {
        return in_array((int) $value, [
            blockvaleur::POSITION_NONE,
            blockvaleur::POSITION_BELOW_HEADER,
            blockvaleur::POSITION_ABOVE_HEADER,
        ], true);
    }
}
