...
$(window).ready(() => {
  ...
  function styleSVG(imgSrc) {
    const imgTarget = $(`.blockvaleur_product img.svg.invisible[src="${imgSrc}"], .blockvaleur img.svg.invisible[src="${imgSrc}"]`);
    ...
    $.ajax({
      ...
      success(data) {
        if ($.isXMLDoc(data)) {
          ...
          $svg.find('path[fill]').attr('fill', window.blv_icon_color);
          $svg.find('path:not([fill])').css('fill', window.blv_icon_color);
  ...
  const imgSrcSvg = $('.blockvaleur_product img.svg, .blockvaleur img.svg').map(function getSrcAttr() {
