...
$(window).ready(() => {
  ...
  new Sortable(document.getElementById('list-blockvaleur'), {
    animation: 150,
    ...
    onUpdate() {
      ...
      $.ajax({
        type: 'POST',
        dataType: 'JSON',
        url: window.blv_controller_block_url,
        data: {
          ajax: true,
  ...
  $(document).on('click', '.blv-listing-row .switch-input', (e) => {
    const switchIsOn = $(e.target).hasClass('-checked');
    const status = switchIsOn ? 1 : 0;

    $(e.target).parent().find('.blv_switch_text').hide();
    if (switchIsOn) {
    ...
    $.ajax({
      ...
      async: false,
      data: {
        controller: window.blv_controller_block,
        action: 'changeBlockStatus',
        idpsr: $(e.target).parent().attr('data-cart_blockvaleur_id'),
        status,
        ajax: true,
      },
      success: (data) => {
  ...
  $(document).on('click', 'body', (e) => {
    ...
    const isPopin = $(e.target).closest('#blockvaleur_block').length;

    if (!isInside && !isPopin) {
      $('#blockvaleur_block').fadeOut(300);
    }
  });

  // Tab Content : Edit : Modify icon : Tabs
  $(document).on('click', '#blockvaleur_block .category_select div img', (e) => {
    const category = $(e.target).attr('data-id');
    // Change the tab
    $('#blockvaleur_block .category_select div').removeClass('active');
    $(e.target).parent().addClass('active');
    // Change the tab content
    $('#blockvaleur_block .category_blockvaleur').removeClass('active');
    $(`#blockvaleur_block .cat_${category}`).addClass('active');
  });

  // Tab Content : Edit : Select icon
  $(document).on('click', '#blockvaleur_block .category_blockvaleur .svg', (e) => {
    const svg = $(e.target)[0].outerHTML;

    // Popin : select the icon
    $('#blockvaleur_block .category_blockvaleur img.svg.selected').removeClass('selected');
  ...
  $(document).on('click', '#blockvaleur_block .select_none', () => {
    const psrPicto = $('.psr-picto:visible');
    psrPicto.attr('src', 'undefined').hide();

    // Un-select icon in the popin
    $('#blockvaleur_block .category_blockvaleur img.svg').removeClass('selected');
  ...
  $(document).on('change', 'input[name^="BLV_REDIRECTION_"]', (e) => {
  ...
  $(document).on('click', '#saveContentConfiguration', function editTabContentSave() {
    ...
    const imgIcon = $('.blv_picto_showing:visible img.psr-picto');
    ...
    $('.show-rea-block.active .content_by_lang').each((index, elem) => {
      ...
      if (!minimalData && lang === window.blv_lang && dataType === 'title' && dataToSave[lang][dataType].length > 0) {
        minimalData = true;
      }
    });
    ...
    formData.append('typelink', $(`input[name="BLV_REDIRECTION_${blockId}"]:checked`).val());
  ...
  new Vue({
    ...
    methods: {
      ...
      isActive(item) {
        ...
        $('.blv_menu').addClass('addons-hide');
        $(`.blv_menu#${item}`).removeClass('addons-hide');
  ...
  $(document).on(
    'change',
    'input[name="BLV_HOOK_CHECKOUT"],input[name="BLV_HOOK_HEADER"],input[name="BLV_HOOK_FOOTER"],input[name="BLV_HOOK_PRODUCT"]',
    function updatePosition() {
      let selector;

      switch ($(this).attr('name')) {
        case 'BLV_HOOK_CHECKOUT':
          selector = 'checkout';
          break;
        case 'BLV_HOOK_HEADER':
          selector = 'header';
          break;
        case 'BLV_HOOK_FOOTER':
          selector = 'footer';
          break;
        case 'BLV_HOOK_PRODUCT':
          selector = 'product';
          break;
        default:
          selector = '';
