...
require dirname(__FILE__) . '/BlockCategorietoplinks.php';

class BlockCategorie extends Module implements WidgetInterface
{
    /**
    ...
    const MENU_JSON_CACHE_KEY = 'MOD_BLOCKCATEGORIE_MENU_JSON';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('Categories for the homepage', [], 'Modules.Blockcategorie.Admin');
        $this->description = $this->trans('Categories for the homepage with image, text and button.', [], 'Modules.Blockcategorie.Admin');
        $this->ps_versions_compliancy = ['min' => '1.7.1.0', 'max' => _PS_VERSION_];
    }
    
    protected function updateTableCategory($action = "install")
    {
        $error=0;
        $sql = [];
        
        if ($action == 'install'){
            $sql[] = 'ALTER TABLE `' . _DB_PREFIX_ . 'category_lang` ADD COLUMN `hp_description` text DEFAULT NULL';
               
        } elseif ($action == 'uninstall') {
            if ($this->columnExists('hp_description', 'category_lang')) {
                $sql[] = 'ALTER TABLE `' . _DB_PREFIX_ . 'category_lang` DROP COLUMN `hp_description`';
            }
        }
    ...
    private function columnExists($columnName, $table = "category")
    {
        $db = Db::getInstance();
    ...
    public function install($delete_params = true)
    {
        ...
        if ($delete_params) {
            if ($this->uninstallPrestaShop16Module()) {
                Configuration::deleteByName('MOD_BLOCKCATEGORIE_SEARCH');

                return true;
            }
            if (!$this->installDb() || !Configuration::updateGlobalValue('MOD_BLOCKCATEGORIE_ITEMS', 'CAT3,CAT6,CAT9')) {
                return false;
            }
        }
    ...
    public function installDb()
    {
		...
		CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'blockcategorie` (
			`id_blockcategorie` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = ' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8mb4;') &&
            Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'blockcategorie_lang` (
			`id_blockcategorie` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_blockcategorie` , `id_lang`, `id_shop`)
		) ENGINE = ' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8mb4;');
    }
    ...
    public function uninstall($delete_params = true)
    {
        ...
        $this->clearMenuCache();
        $updateTableCategory = $this->updateTableCategory('uninstall');

        if ($delete_params) {
            if (!$this->uninstallDB() || !Configuration::deleteByName('MOD_BLOCKCATEGORIE_ITEMS')) {
                return false;
            }
        }
    ...
    {
        Db::getInstance()->execute('DROP TABLE `' . _DB_PREFIX_ . 'blockcategorie`');
        Db::getInstance()->execute('DROP TABLE `' . _DB_PREFIX_ . 'blockcategorie_lang`');
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitBlocktopmenu')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                if (count($shops) == 1) {
                    if (is_array($items) && count($items)) {
                        $updated = Configuration::updateValue('MOD_BLOCKCATEGORIE_ITEMS', (string) implode(',', $items), false, (int) $shop_group_id, (int) $shop_id);
                    } else {
                        $updated = Configuration::updateValue('MOD_BLOCKCATEGORIE_ITEMS', '', false, (int) $shop_group_id, (int) $shop_id);
                    }
                }
            ...
            if (!count($errors_update_shops)) {
                ...
                $this->_html .= $this->displayError(sprintf($this->trans('Unable to update settings for the following shop(s): %s', [], 'Modules.Blockcategorie.Admin'), implode(', ', $errors_update_shops)));
            }
            ...
            if (Tools::isSubmit('submitBlocktopmenuLinks')) {
                ...
                if ($count_links_label || $count_label) {
                    if (!$count_links_label) {
                        $this->_html .= $this->displayError($this->trans('Please complete the "Link" field.', [], 'Modules.Blockcategorie.Admin'));
                    } elseif (!$count_label) {
                        $this->_html .= $this->displayError($this->trans('Please add a label.', [], 'Modules.Blockcategorie.Admin'));
                    } elseif (!isset($labels[$default_language])) {
                        $this->_html .= $this->displayError($this->trans('Please add a label for your default language.', [], 'Modules.Blockcategorie.Admin'));
                    } else {
                        $shops = Shop::getContextListShopID();

                        foreach ($shops as $shop_id) {
                            $added = BlockCategorietoplinks::add($links_label, $labels, Tools::getValue('new_window', 0), (int) $shop_id);
                        ...
                        if (!count($errors_add_link)) {
                            $this->_html .= $this->displayConfirmation($this->trans('The link has been added.', [], 'Modules.Blockcategorie.Admin'));
                        } else {
                            $this->_html .= $this->displayError($this->trans('Unable to add link for the following shop(s): %s', [implode(', ', $errors_add_link)], 'Modules.Blockcategorie.Admin'));
                        }
                    }
                }
                $update_cache = true;
            } elseif (Tools::isSubmit('deleteblockcategorie')) {
                $errors_delete_link = [];
                $id_blockcategorie = Tools::getValue('id_blockcategorie', 0);
                $shops = Shop::getContextListShopID();

                foreach ($shops as $shop_id) {
                    $deleted = BlockCategorietoplinks::remove($id_blockcategorie, (int) $shop_id);
                    Configuration::updateValue('MOD_BLOCKCATEGORIE_ITEMS', str_replace(['LNK' . $id_blockcategorie . ',', 'LNK' . $id_blockcategorie], '', Configuration::get('MOD_BLOCKCATEGORIE_ITEMS')));
                ...
                if (!count($errors_delete_link)) {
                    $this->_html .= $this->displayConfirmation($this->trans('The link has been removed.', [], 'Modules.Blockcategorie.Admin'));
                } else {
                    $this->_html .= $this->displayError($this->trans('Unable to remove link for the following shop(s): %s', [implode(', ', $errors_delete_link)], 'Modules.Blockcategorie.Admin'));
                }

                $update_cache = true;
            } elseif (Tools::isSubmit('updateblockcategorie')) {
                $id_blockcategorie = (int) Tools::getValue('id_blockcategorie', 0);
                ...
                if (Tools::isSubmit('updatelink')) {
                    ...
                    BlockCategorietoplinks::update($link, $label, $new_window, (int) $id_shop, (int) $id_blockcategorie);
                    $this->_html .= $this->displayConfirmation($this->trans('The link has been edited.', [], 'Modules.Blockcategorie.Admin'));
                }
                $update_cache = true;
            }
        }
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, BlockCategorietoplinks::gets((int) $id_lang, null, (int) $shop_id));
        }
    ...
    protected function getWarningMultishopHtml()
    {
        return '<p class="alert alert-warning">' .
                    $this->trans('You cannot manage top menu items from a "All Shops" or a "Group Shop" context, select directly the shop you want to edit', [], 'Modules.Blockcategorie.Admin') .
                '</p>';
    }
    ...
    protected function getCurrentShopInfoMsg()
    {
        ...
        if (Shop::getContext() == Shop::CONTEXT_SHOP) {
            $shop_info = $this->trans('The modifications will be applied to shop: %s', [$this->context->shop->name], 'Modules.Blockcategorie.Admin');
        } else {
            if (Shop::getContext() == Shop::CONTEXT_GROUP) {
                $shop_info = $this->trans('The modifications will be applied to this group: %s', [Shop::getContextShopGroup()->name], 'Modules.Blockcategorie.Admin');
            } else {
                $shop_info = $this->trans('The modifications will be applied to all shops', [], 'Modules.Blockcategorie.Admin');
            }
        }
        ...
        if (is_array($items) && count($items)) {
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    $shop_group_id = Shop::getGroupFromShop($shop_id);
                    $conf .= (string) ($key > 1 ? ',' : '') . Configuration::get('MOD_BLOCKCATEGORIE_ITEMS', null, $shop_group_id, $shop_id);
                }
            } else {
                $shop_id = (int) $shops[0];
                $shop_group_id = Shop::getGroupFromShop($shop_id);
                $conf = Configuration::get('MOD_BLOCKCATEGORIE_ITEMS', null, $shop_group_id, $shop_id);
            }
    ...
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                ...
                case 'ALLMAN':
                    $html .= '<option selected="selected" value="ALLMAN0">' . $this->trans('All brands', [], 'Modules.Blockcategorie.Admin') . '</option>' . PHP_EOL;
                    break;
                ...
                case 'ALLSUP':
                    $html .= '<option selected="selected" value="ALLSUP0">' . $this->trans('All suppliers', [], 'Modules.Blockcategorie.Admin') . '</option>' . PHP_EOL;
                    break;
                ...
                case 'LNK':
                    $link = BlockCategorietoplinks::get((int) $id, (int) $id_lang, (int) $id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = BlockCategorietoplinks::get($link[0]['id_blockcategorie'], (int) $default_language, (int) Shop::getContextShopID());
                        }
                        $html .= '<option selected="selected" value="LNK' . (int) $link[0]['id_blockcategorie'] . '">' . Tools::safeOutput($link[0]['label']) . '</option>';
                    }
                    break;
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'ALLMAN':
                    ...
                    $root_node['children'][] = $this->makeNode([
                        ...
                        'label' => $this->trans('All brands', [], 'Modules.Blockcategorie.Admin'),
                ...
                case 'ALLSUP':
                    ...
                    $root_node['children'][] = $this->makeNode([
                        ...
                        'label' => $this->trans('All suppliers', [], 'Modules.Blockcategorie.Admin'),
                ...
                case 'SHOP':
                    ...
                    break;
                case 'LNK':
                    $link = BlockCategorietoplinks::get($id, $id_lang, $id_shop);
                    if (!empty($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = BlockCategorietoplinks::get($link[0]['id_blockcategorie'], $default_language, (int) Shop::getContextShopID());
                        }
                        $root_node['children'][] = $this->makeNode([
                            'type' => 'link',
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $node['hp_description'] = $category['hp_description'];
    ...
    public function hookActionCategoryFormBuilderModifier(array $params)
    {
        ...
        $formBuilder
            ->add('hp_description', TranslateType::class, [
                'type' => FormattedTextareaType::class,
                'locales' => $locales,
                'label' => $this->trans('Texte pour l\'affichage de la catégorie sur la homepage', [], 'Modules.Blockcategorie.Admin'),
                'hideTabs' => false,
                'required' => false,
                'options' => [
                    'required' => false,
        ...
        $category = new Category($params['id'], Context::getContext()->language->id);
        $params['data']['hp_description'] = $category->hp_description;
    ...
    protected function updateData(int $id_category, array $data)
    {
        ...
        if (!empty($data['hp_description'])) {           
            $category->hp_description = $data['hp_description'];
        }
    ...
    {
        $dir = _PS_CACHE_DIR_ . 'blockcategorie';
    ...
    {
        $this->cleanMenuCacheDirectory(_PS_CACHE_DIR_ . 'blockcategorie');
    }
    ...
    {
        /** @var array<int, array{'id_blockcategorie': int, 'id_shop': int, 'new_window': int}> $blockcategorie */
        $blockcategorie = Db::getInstance()->executeS(
            'SELECT * FROM ' . _DB_PREFIX_ . 'blockcategorie '
            . 'WHERE id_shop = ' . (int) $params['old_id_shop']
        );

        foreach ($blockcategorie as $id => $link) {
            Db::getInstance()->execute('
				INSERT IGNORE INTO ' . _DB_PREFIX_ . 'blockcategorie (id_blockcategorie, id_shop, new_window)
				VALUES (null, ' . (int) $params['new_id_shop'] . ', ' . (int) $link['new_window'] . ')');

            $blockcategorie[$id]['new_id_blockcategorie'] = Db::getInstance()->Insert_ID();
        }

        foreach ($blockcategorie as $id => $link) {
            $lang = Db::getInstance()->executeS('
					SELECT id_lang, ' . (int) $params['new_id_shop'] . ', label, link
					FROM ' . _DB_PREFIX_ . 'blockcategorie_lang
					WHERE id_blockcategorie = ' . (int) $link['id_blockcategorie'] . ' AND id_shop = ' . (int) $params['old_id_shop']);

            foreach ($lang as $l) {
                Db::getInstance()->execute('
					INSERT IGNORE INTO ' . _DB_PREFIX_ . 'blockcategorie_lang (id_blockcategorie, id_lang, id_shop, label, link)
					VALUES (' . (int) $link['new_id_blockcategorie'] . ', ' . (int) $l['id_lang'] . ', ' . (int) $params['new_id_shop'] . ', ' . (int) $l['label'] . ', ' . (int) $l['link'] . ' )');
            }
        }
    }
    ...
    public function renderForm()
    {
        ...
        if (count($shops) == 1) {
            $fields_form = [
                'form' => [
                    'legend' => [
                        'title' => $this->trans('Menu Top Link', [], 'Modules.Blockcategorie.Admin'),
                        'icon' => 'icon-link',
                    ],
                    'input' => [
                        [
            ...
            $fields_form = [
                'form' => [
                    ...
                    'info' => '<div class="alert alert-warning">' .
                        $this->trans('All active products combinations quantities will be changed', [], 'Modules.Blockcategorie.Admin') . '</div>',
                    'submit' => [
                        'name' => 'submitBlocktopmenu',
                        'title' => $this->trans('Save', [], 'Admin.Actions'),
                    ],
    ...
    public function renderAddForm()
    {
        $fields_form = [
            'form' => [
                'legend' => [
                    'title' => Tools::getValue('id_blockcategorie') && !Tools::isSubmit('updatelink') ?
                        $this->trans('Update link', [], 'Modules.Blockcategorie.Admin') : $this->trans('Add a new link', [], 'Modules.Blockcategorie.Admin'),
                    'icon' => 'icon-link',
                ],
                'input' => [
                    [
        ...
        if (Tools::getValue('id_blockcategorie') && !Tools::isSubmit('updatelink')) {
            $fields_form['form']['submit'] = [
                'name' => 'updateblockcategorie',
                'title' => $this->trans('Update', [], 'Admin.Actions'),
            ];
        }

        if (Tools::isSubmit('updateblockcategorie')) {
            $fields_form['form']['input'][] = ['type' => 'hidden', 'name' => 'updatelink'];
            $fields_form['form']['input'][] = ['type' => 'hidden', 'name' => 'id_blockcategorie'];
    ...
    public function renderChoicesSelect()
    {
        ...
        $html .= '<optgroup label="' . $this->trans('CMS', [], 'Modules.Blockcategorie.Admin') . '">';
        ...
        $html .= '<option value="ALLSUP0">' . $this->trans('All suppliers', [], 'Modules.Blockcategorie.Admin') . '</option>';
        ...
        $html .= '<option value="ALLMAN0">' . $this->trans('All brands', [], 'Modules.Blockcategorie.Admin') . '</option>';
        ...
        // BEGIN Shops
        if (Shop::isFeatureActive()) {
            $html .= '<optgroup label="' . $this->trans('Shops', [], 'Modules.Blockcategorie.Admin') . '">';
        ...
        $html .= '<option value="PRODUCT" style="font-style:italic">' . $spacer . $this->trans('Choose product ID', [], 'Modules.Blockcategorie.Admin') . '</option>';
        $html .= '</optgroup>';

        // BEGIN Menu Top Links
        $html .= '<optgroup label="' . $this->trans('Menu Top Links', [], 'Modules.Blockcategorie.Admin') . '">';
        $links = BlockCategorietoplinks::gets($this->context->language->id, null, (int) Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $default_language = Configuration::get('PS_LANG_DEFAULT');
                $link = BlockCategorietoplinks::get($link['id_blockcategorie'], $default_language, (int) Shop::getContextShopID());
                if (!in_array('LNK' . (int) $link[0]['id_blockcategorie'], $items)) {
                    $html .= '<option value="LNK' . (int) $link[0]['id_blockcategorie'] . '">' . $spacer . Tools::safeOutput($link[0]['label']) . '</option>';
                }
            } elseif (!in_array('LNK' . (int) $link['id_blockcategorie'], $items)) {
                $html .= '<option value="LNK' . (int) $link['id_blockcategorie'] . '">' . $spacer . Tools::safeOutput($link['label']) . '</option>';
            }
        }
        $html .= '</optgroup>';
        $html .= '</select>';
    ...
    public function getAddLinkFieldsValues()
    {
        ...
        if (Tools::isSubmit('updateblockcategorie')) {
            $link = BlockCategorietoplinks::getLinkLang(Tools::getValue('id_blockcategorie'), (int) Shop::getContextShopID());
        ...
        $fields_values = [
            ...
            'id_blockcategorie' => Tools::getValue('id_blockcategorie'),
        ];
    ...
    public function renderList()
    {
        ...
        foreach ($shops as $shop_id) {
            $links = array_merge($links, BlockCategorietoplinks::gets((int) $this->context->language->id, null, (int) $shop_id));
        }

        $fields_list = [
            'id_blockcategorie' => [
                'title' => $this->trans('Link ID', [], 'Modules.Blockcategorie.Admin'),
                'type' => 'text',
            ],
            'name' => [
        ...
        $helper->identifier = 'id_blockcategorie';
        $helper->table = 'blockcategorie';
        $helper->actions = ['edit', 'delete'];
        $helper->show_toolbar = false;
        $helper->module = $this;
        $helper->title = $this->trans('Link list', [], 'Modules.Blockcategorie.Admin');
        $helper->token = Tools::getAdminTokenLite('AdminModules');
    ...
    public function renderWidget($hookName, array $configuration)
    {
        ...
        return $this->fetch('module:blockcategorie/blockcategorie.tpl');
    }
}
