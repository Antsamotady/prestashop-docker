...
function add()
{
	$("#availableItems option:selected").each(function(i){
		...
		if (val == "PRODUCT")
		{
			val = prompt('{l s="Indicate the ID number for the product" d='Modules.Blockcategorie.Admin' js=1}');
			if (val == null || val == "" || isNaN(val))
				return;
			text = '{l s="Product ID #" d='Modules.Blockcategorie.Admin' js=1}'+val;
			val = "PRD"+val;
		}
...
function move(up)
{
        ...
        if (tomove.length >1)
        {
                alert('{l s="Please select just one item" d='Modules.Blockcategorie.Admin'}');
                return false;
        }
        if (up)
...
{block name="input"}
    {if $input.type == 'link_choice'}
	    <div class="row">
	    	<div class="col-sm-2">
	    		<h4 style="margin-top:5px;">{l s='Change position' d='Modules.Blockcategorie.Admin'}</h4>
	    	...
	    	<div class="col-sm-5">
	    		<h4 style="margin-top:5px;">{l s='Selected items' d='Modules.Blockcategorie.Admin'}</h4>
	    		{$selected_links}
	    	</div>
	    	<div class="col-sm-5">
	    		<h4 style="margin-top:5px;">{l s='Available items' d='Modules.Blockcategorie.Admin'}</h4>
	    		{$choices}
	    	</div>
	    ...
	    <div class="row">
	    	...
	    	<div class="col-sm-5"><a href="#" id="removeItem" class="btn btn-default"><i class="icon-arrow-right"></i> {l s='Remove' d='Modules.Blockcategorie.Admin'}</a></div>
